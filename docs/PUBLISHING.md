...
This repo publishes eight packages — `DuonDevKit.Core`, `DuonDevKit.EntityFrameworkCore`,
`DuonDevKit.AspNetCore`, `DuonDevKit.Dapper`, `DuonDevKit.Caching`, `DuonDevKit.Jwt`,
`DuonDevKit.Validation`, and `DuonDevKit.Templates` — to NuGet.org via GitHub Actions,
...
1. Sign in to [nuget.org](https://www.nuget.org) with an account that owns (or will own) all eight
   package IDs.
...
3. Create a Trusted Publishing policy for each package (`DuonDevKit.Core`,
   `DuonDevKit.EntityFrameworkCore`, `DuonDevKit.AspNetCore`, `DuonDevKit.Dapper`,
   `DuonDevKit.Caching`, `DuonDevKit.Jwt`, `DuonDevKit.Validation`, `DuonDevKit.Templates`) pointing
   at:
...
2. Bump `<Version>` in whichever of `DuonDevKit.Core/DuonDevKit.Core.csproj`,
   ...
   `DuonDevKit.Dapper/DuonDevKit.Dapper.csproj`, `DuonDevKit.Caching/DuonDevKit.Caching.csproj`,
   `DuonDevKit.Jwt/DuonDevKit.Jwt.csproj`, `DuonDevKit.Validation/DuonDevKit.Validation.csproj`, or
...
4. Tag and push. Tags aren't tied 1:1 to a package version — each push of a `v*` tag just triggers
   the workflow, which packs and pushes all eight projects (`--skip-duplicate` no-ops whichever
...
dotnet pack DuonDevKit.Caching/DuonDevKit.Caching.csproj --configuration Release --output ./nupkgs
