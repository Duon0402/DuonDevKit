...
This repo publishes four packages — `DuonDevKit.Core`, `DuonDevKit.EntityFrameworkCore`,
`DuonDevKit.AspNetCore`, and `DuonDevKit.Dapper` — to NuGet.org via GitHub Actions, authenticated
with **Trusted Publishing (OIDC)**. No static API key is stored in GitHub Secrets.

## One-time setup: Trusted Publishing policy on NuGet.org

1. Sign in to [nuget.org](https://www.nuget.org) with an account that owns (or will own) all four
   package IDs.
...
3. Create a Trusted Publishing policy for each package (`DuonDevKit.Core`,
   `DuonDevKit.EntityFrameworkCore`, `DuonDevKit.AspNetCore`, `DuonDevKit.Dapper`) pointing at:
...
2. Bump `<Version>` in whichever of `DuonDevKit.Core/DuonDevKit.Core.csproj`,
   ...
   `DuonDevKit.AspNetCore/DuonDevKit.AspNetCore.csproj`, or
   `DuonDevKit.Dapper/DuonDevKit.Dapper.csproj` changed.
3. Commit the version bump.
4. Tag and push. Tags aren't tied 1:1 to a package version — each push of a `v*` tag just triggers
   the workflow, which packs and pushes all four projects (`--skip-duplicate` no-ops whichever
   package(s) didn't change this release):
   ```
   git tag vX.Y.Z
   git push origin vX.Y.Z
   ```
5. The `publish` workflow runs automatically: build → test → pack → push all four `.nupkg` files to
...
`DuonDevKit.Core` dependency versions look right before pushing a tag.
